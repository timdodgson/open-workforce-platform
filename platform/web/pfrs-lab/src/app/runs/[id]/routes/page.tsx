...
export default async function RoutesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!solutionContent) {
      return (
        <RunPageShell
          title="Route Viewer"
          empty
          emptyMessage="No route data available. Run solve-cvrp with --run-label to generate solution.json."
        >
          {null}
        </RunPageShell>
      );
    ...
    try {
      ...
      return (
        <RunPageShell title="Route Viewer" error="Error parsing solution.json">
          {null}
        </RunPageShell>
      );
    }
    ...
    return (
      <RunPageShell title="Route Viewer">
