  ...
  if (loading) {
    return (
      <Card title="Predictions">
        <TabSpinner label="Loading predictions…" />
      </Card>
    );
  }
    ...
    return (
      <Card title="Predictions">
        <p className="text-xs text-gray-500 text-center py-12">
          {error ? `Failed to load: ${error}` : 'No prediction data. Run worker_model.predict to shard predictions per run.'}
        </p>
      </Card>
    );
  }
...
export function WhatIfTabClient() {
  ...
  if (error || !data?.predictions.length) {
    return (
      <Card title="What-If Lab">
        <p className="text-xs text-gray-500 text-center py-12">
          {error ? `Failed to load: ${error}` : 'No prediction data for simulation.'}
        </p>
      </Card>
    );
  }
