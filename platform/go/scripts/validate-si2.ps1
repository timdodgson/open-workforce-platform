...
foreach ($policy in $policies) {
    foreach ($seed in $seeds) {
        ...
        go run ./cmd/owp solve-cvrp --instance ../../examples/cvrp/A-n32-k5.vrp --mode sa --iterations 500000 --policy-mode $policy --policy-dir $policyDir --seed $seed --run-label $label --pfrs-storage local 2>&1 | Out-Null
    }
}
...
foreach ($policy in $policies) {
    foreach ($seed in $seeds) {
        ...
        Write-Host "  $label" -ForegroundColor Gray
        go run ./cmd/owp solve-cvrp --instance ../../examples/cvrp/A-n32-k5.vrp --mode portfolio --iterations 500000 --policy-mode $policy --policy-dir $policyDir --seed $seed --run-label $label --pfrs-storage local 2>&1 | Out-Null
    }
}
...
foreach ($policy in $policies) {
    foreach ($seed in $seeds) {
        ...
        Write-Host "  $label" -ForegroundColor Gray
        go run ./cmd/owp solve-jobshop --instance internal/infrastructure/jobshop/testdata/la01.txt --mode tabu --iterations 100000 --policy-mode $policy --policy-dir $policyDir --seed $seed --run-label $label --pfrs-storage local 2>&1 | Out-Null
    }
}
...
foreach ($policy in $policies) {
    foreach ($seed in $seeds) {
        ...
        Write-Host "  $label" -ForegroundColor Gray
        go run ./cmd/owp solve-jobshop --instance internal/infrastructure/jobshop/testdata/la01.txt --mode portfolio --iterations 100000 --policy-mode $policy --policy-dir $policyDir --seed $seed --run-label $label --pfrs-storage local 2>&1 | Out-Null
    }
}
...
foreach ($policy in $policies) {
    foreach ($seed in $seeds) {
        ...
        Write-Host "  $label" -ForegroundColor Gray
        go run ./cmd/owp solve-vrptw --instance ../../examples/vrptw/C101.txt --mode sa --iterations 100000 --policy-mode $policy --policy-dir $policyDir --seed $seed --run-label $label --pfrs-storage local 2>&1 | Out-Null
    }
}
...
foreach ($policy in $policies) {
    foreach ($seed in $seeds) {
        ...
        Write-Host "  $label" -ForegroundColor Gray
        go run ./cmd/owp solve-vrptw --instance ../../examples/vrptw/C101.txt --mode portfolio --iterations 100000 --policy-mode $policy --policy-dir $policyDir --seed $seed --run-label $label --pfrs-storage local 2>&1 | Out-Null
    }
}
