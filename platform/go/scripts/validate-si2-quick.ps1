...
foreach ($policy in $policies) {
    ...
    go run ./cmd/owp solve-cvrp --instance ../../examples/cvrp/A-n32-k5.vrp --mode sa --iterations 50000 --policy-mode $policy --policy-dir $policyDir --seed 42 --run-label $label --storage s3 2>&1 | Out-Null
}

foreach ($policy in $policies) {
    $label = "val-quick-cvrp-portfolio-${policy}-s42"
    Write-Host "  $label" -ForegroundColor Gray
    go run ./cmd/owp solve-cvrp --instance ../../examples/cvrp/A-n32-k5.vrp --mode portfolio --iterations 50000 --policy-mode $policy --policy-dir $policyDir --seed 42 --run-label $label --storage s3 2>&1 | Out-Null
}
