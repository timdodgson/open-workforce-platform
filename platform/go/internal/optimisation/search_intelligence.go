...
//
// SearchIntelligence combines all three assist styles into a single entry point.
// Solvers check which interfaces are available and use the appropriate one.
//
// A nil value for any interface means that assist style is not active.

// SearchIntelligence holds the AI advisors for all integration styles.
type SearchIntelligence struct {
	// Worker-level assist (beam search).
	Worker WorkerAssist

	// Portfolio-level assist (multi-strategy).
	Portfolio PortfolioAssist

	// Search-level assist (single algorithm).
	Search SearchAssist

	// Mode controls whether recommendations are acted upon.
	// "off" = no AI
...
// IsActive returns true if any assist style is configured and mode is not "off".
func (si *SearchIntelligence) IsActive() bool {
	if si == nil || si.Mode == "off" || si.Mode == "" {
		return false
	}
